...
for row in potus:
    ...
    row[2] = dt.datetime.strptime(app_start_date, date_format)
...
for row in potus:
    app_start_date = row[2]
    date_string = app_start_date.strftime('%B, %Y')
    if date_string in visitors_per_month:
        visitors_per_month[date_string] += 1
    else: 
        visitors_per_month[date_string] = 1
